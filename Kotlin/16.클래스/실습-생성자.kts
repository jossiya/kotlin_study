...
// 주생성자 -> 풀버젼 (생략이 없는 버젼)
class User1 constructor(name: String) { // 16.클래스 네이밍은 대문자로 시작한다
    val userName: String // 16.클래스 속성 (프로퍼티,property)은 init블럭에서 초기화 된다
...
class User5 constructor(age: Int, name: String) {
    val age: Int // 16.클래스 속성
    val name: String

    init {
        this.age = age //this 는 16.클래스 자체를 의미한다 (User5) ->User5의age
        this.name = name
    }
}
// .의 의미
...
// 16.클래스 속성에서 초기화를 시켜주쥐 않아도 되는 이유
